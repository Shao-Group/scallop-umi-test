...
def load_data(tool):
    ...
    m = np.zeros([p1_size * p2_size,369])
    p = np.zeros([p1_size * p2_size,369])
...
def plot_2d(scallop2_match, stringtie2_match, scallop_match, scallop2_pre, stringtie2_pre, scallop_pre):
    ...
    for i in range(0, p1_size):
        ...
        ax.set(ylim=[900, 2500],xlim=[25, 90])
        plt.xticks(np.arange(25, 91, 30), fontsize=24, horizontalalignment='center')
        plt.yticks(np.arange(900, 2501, 800), fontsize=24)
        ...
        plt.text(26, 1060, text, fontsize=20)
        plt.text(10, 2420, idx[i], fontsize=30)
        figure_name = './Mouse-Fibroblast/figure/Mouse-Fibroblast-vary-para-' + str(p1[i]) + '-multi-2d.pdf'
...
def get_adjusted(idx1, idx2, scallop2_match, scallop2_pre, stringtie2_match, stringtie2_pre, scallop_match, scallop_pre):
    ...
    for i in hi:
        file1 = './Mouse-Fibroblast/stringtie2/' + str(i) + '.' + str(p1[idx1]) + '-' + str(p2[idx2]) + '.stringtie2.me.roc-cor'
        file2 = './Mouse-Fibroblast/stringtie2/' + str(i) + '.' + str(p1[idx1]) + '-' + str(p2[idx2]) + '.stringtie2.me.roc-pre'
    ...
    for i in hni:
        file1 = './Mouse-Fibroblast/scallop2/' + str(i) + '.' + str(p1[idx1]) + '-' + str(p2[idx2]) + '.scallop2.me.roc-cor'
        file2 = './Mouse-Fibroblast/scallop2/' + str(i) + '.' + str(p1[idx1]) + '-' + str(p2[idx2]) + '.scallop2.me.roc-pre'
        cor = np.loadtxt(file1)
        pre = np.loadtxt(file2)
    ...
    for i in hi:
        file1 = './Mouse-Fibroblast/scallop/' + str(i) + '.' + str(p1[idx1]) + '-' + str(p2[idx2]) + '.scallop.me.roc-cor'
        file2 = './Mouse-Fibroblast/scallop/' + str(i) + '.' + str(p1[idx1]) + '-' + str(p2[idx2]) + '.scallop.me.roc-pre'
        cor = np.loadtxt(file1)
        pre = np.loadtxt(file2)
        cur_umi = scallop2_match[i-1]
        for j in range(0, len(cor)):
...
def plot_adjusted_bar(scallop2_match, stringtie2_match, scallop_match, scallop2_pre, stringtie2_pre, scallop_pre):
    ...
    for i in range(0, p1_size):
        ...
        figure_name = './Mouse-Fibroblast/figure/MF-vary-para-adjusted' + str(p1[i]) +'-multi.pdf'
        plt.savefig(figure_name, bbox_inches='tight')
