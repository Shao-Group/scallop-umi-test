...
def plot_2d(scallop2_match, stringtie2_match, scallop_match, scallop2_pre, stringtie2_pre, scallop_pre):
    ...
    ax.set(ylim=[0, 10000],xlim=[20, 40])
    plt.xticks(np.arange(20, 41, 10), fontsize=24, horizontalalignment='center')
    ...
    plt.text(22, 1666, 'ENCODE10-HISAT2', fontsize=20)
    plt.text(15, 9600, ' ', fontsize=30)
    figure_name = "./ENCODE10/figure/encode10-hisat-class-2d.pdf"
    ...
    plt.ylabel("# Matching Transcripts", fontsize=24)
    plt.xlabel("Precision (%)", fontsize=24)
    plt.text(23, 1666, 'ENCODE10-STAR', fontsize=20)
    plt.text(15, 9600, ' ', fontsize=30)
    figure_name = "./ENCODE10/figure/encode10-star-class-2d.pdf"
    plt.savefig(figure_name, bbox_inches='tight')
    plt.show()
...
def plot_adjusted_bar(dataset, scallop2_match, scallop2_pre, stringtie2_match, stringtie2_pre, scallop_match, scallop_pre):
    ...
    for i in range(0, 3):
        ...
        if(i == 1):
            ax.set(ylim=[0,60])
            plt.yticks(np.arange(0,61,30), fontsize=32)
        elif(i == 2):
            ax.set(ylim=[0,60])
            plt.yticks(np.arange(0, 62, 30), fontsize=32)
        plt.yticks(rotation = 90)
        ...
        figure_name = "./ENCODE10/figure/" + dataset + "-" + levels[i] + "-adjusted-precision-class.pdf"
        plt.savefig(figure_name, bbox_inches='tight')
        plt.show()
