...
def load_data(tool, t_type):

    scallop2 = np.zeros([369,16]) # 1 total + 15 code class
    
    dirPath = './Mouse-Fibroblast/scallop2/scallop2-'+ t_type + '/'
    
    for i in range(0, 369):
        file1 = dirPath + str(i+1) + '.codes'
        codes = []
...
def plot_table(scallop2, t_type):
    ave = np.zeros([1,16])
    ...
    for i in range(0,16):
        ave[0][i] = np.around(np.mean(scallop2_t[i]), 2)
...
def plot_bar(nf_ave, nf_sd, f_ave, f_sd):
    ...
    ax.set(ylim=[0, 76])
    ax.set(xlim=[-0.6, 8.5])
    plt.yticks(np.arange(0, 78, 25), fontsize=16)
    ...
    plt.text(-2.5,75,idx, fontsize=18)
    figure_name = "./Mouse-Fibroblast/figure/MF-fragments.pdf"
    plt.savefig(figure_name, bbox_inches='tight')
    plt.show()
    
def main():
    nonfull_scallop2 = load_data('scallop2', 'non-full')
    plot_table(nonfull_scallop2, 'non-full')

    full_scallop2 = load_data('scallop2', 'full')
    plot_table(full_scallop2, 'full')
    
    nf_ave, nf_sd = get_pre(nonfull_scallop2)
    f_ave, f_sd = get_pre(full_scallop2)
    plot_bar(nf_ave, nf_sd, f_ave, f_sd)
