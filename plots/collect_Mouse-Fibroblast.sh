...
if [ "A" == "A" ];then
	...
	index=$dir/../data/mouse/salmon.index
        ...
        bam=$dir/../results/Mouse-Fibroblast_index
        mkdir -p $fq
        cd $fq
        ...
        do
                ...
                echo "cd $i" >> $script
                echo "$gtfcuff split-quant $tmap $quant/$i.quant/quant.sf.mod 0.1 $dir/../data/mouse/Mus_musculus.GRCm39.104.gtf" >> $script
                chmod +x $script
	...
	cd $cref
        $gtfcuff split-class $result/scallop2/1-100-0.001.scallop2.me.1-100-0.001.scallop2.gtf.tmap ../Mus_musculus.GRCm39.104.gtf mouse
        for n in 2-3 4-6 7;
        do
                mv mouse.$n.gtf $n.gtf
        done
